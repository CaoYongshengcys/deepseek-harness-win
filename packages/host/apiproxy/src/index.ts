...
import { createApiProxy, DEFAULT_COLD_BLANK_PROBE_MAX_BYTES, DEFAULT_LIST_FILES_MAX_ENTRIES } from './api-proxy.ts'
import {
...
export class ApiProxyService extends Service implements ApiProxy {
  ...
  static Config: z<Config> = z.object({
    ...
    listFilesMaxEntries: z.natural().min(1).default(DEFAULT_LIST_FILES_MAX_ENTRIES),
  })
  ...
  constructor(ctx: Context, config: Config) {
    ...
    const api = createApiProxy(ctx, {
      ...
      ...(config.listFilesMaxEntries === undefined
        ? {}
        : { listFilesMaxEntries: config.listFilesMaxEntries }),
    })
