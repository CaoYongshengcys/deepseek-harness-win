...
import type {
  ApiProxy, ConfigurableProviderView, CredentialView, FileEntry, FileListing, GoalRef, HistoryEntry, HostFrame,
...
/** Default complete-result bound of one `host.listFiles` level (matches the directory-picker browse default). */
export const DEFAULT_LIST_FILES_MAX_ENTRIES = 1000
...
export interface ApiProxyDefaults {
  ...
  /** Complete-result bound of one `host.listFiles` level; defaults to {@link DEFAULT_LIST_FILES_MAX_ENTRIES}. */
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  const listFilesMaxEntries = defaults.listFilesMaxEntries
  ...
  /**
   * List one directory level's files and directories for the workspace file
   * tree. Directories come first (name-sorted), then files (name-sorted); a
   * symlink probes its target to report enterability, and a broken link is
   * reported as a plain file row. The level is bounded at `listFilesMaxEntries`
   * rows, `truncated` flagging a cut. Throws with a plain Error carrying the
   * filesystem reason; the caller maps it onto the wire.
   * @param target - absolute directory to list.
   * @param signal - caller lifetime; an abort rejects before the scan starts or between entries.
   * @returns the bounded listing for the level.
   */
  async function listFilesAt(target: string, signal: AbortSignal): Promise<FileListing> {
    signal.throwIfAborted()
    ...
    for (const dirent of dirents) {
      ...
      if (!isDirectory && dirent.isSymbolicLink()) {
        try {
          ...
          // Broken or cyclic symlink: report it as a non-expandable file row.
          isDirectory = false
        }
      }
      const entry: FileEntry = { name: dirent.name, path, isDirectory, hidden: dirent.name.startsWith('.') }
    ...
    const truncated = entries.length > listFilesMaxEntries
    return { path: target, entries: truncated ? entries.slice(0, listFilesMaxEntries) : entries, truncated }
  }
  ...
  return {
    ...
    host: {
      ...
      async listFiles(request, signal) {
        // An absent path lists the host working directory — the same root
        // session.create falls back to, so the tree matches where an
        // unspecified-cwd session lands.
        const target = resolve(request.payload.path ?? defaults.cwd)
        try {
          return ok(request, await listFilesAt(target, signal))
        } catch (error: unknown) {
          if (signal.aborted) {
