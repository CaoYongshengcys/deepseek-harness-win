...
/** One filesystem entry of a file listing: a file or a directory row. */
export interface FileEntry {
  /** Base name shown in a tree row. */
  name: string
  /** Absolute host path — the client never joins path segments itself. */
  path: string
  /** True for directories and symlinks that resolve to directories (both expandable). */
  isDirectory: boolean
  /** Hidden by the host platform's convention (dot-prefixed on POSIX); the client owns whether to show it. */
  hidden: boolean
}

/** host.listFiles response value: one directory level's files and directories. */
export interface FileListing {
  /** Absolute path of the listed directory. */
  path: string
  /** Direct child entries, directories first then files, each group name-sorted. */
  entries: FileEntry[]
...
export interface HostApi {
   ...
   * tree; an absent path lists the host process working directory. Unlike
   * `listDirectory` (the directory-picker's directories-only browse primitive),
   * this returns both files and directories and is always served (read-only,
   * like `openPath`). An unreadable or missing target fails with
   * `directory-unreadable`. The carrier's request signal follows the caller,
   * stopping the scan on disconnect or timeout.
   */
  listFiles(
    request: RpcRequest<{ path?: string }>,
    signal: AbortSignal,
