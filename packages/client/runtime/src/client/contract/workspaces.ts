 ...
 */
import type { DirectoryListing, FileListing, SessionId, WorkspaceId, WorkspaceView } from '@deepseek-ai/dsh-api-remotes/client'
...
export interface IWorkspaces {
  ...
  /**
   * List one directory level's files and directories for the workspace file tree.
   * @param path - absolute directory to list; absent lists the Host working directory.
   * @param signal - aborts the wire request (and the Host's scan) when the caller supersedes it.
   * @returns the level's file and directory entries (directories first).
   */
