...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const api: ApiProxy = {
    ...
    host: {
      ...
      // The fixture tree models directories only, so every entry is expandable.
      listFiles: (request) => {
        const target = request.payload.path ?? '/tmp/fixture'
        const children = childrenOf(target)
        if (children === undefined) {
          return err(request, { code: 'directory-unreadable', message: `cannot list ${target}: not in the fixture tree`, details: { path: target } })
        }
        return ok(request, {
          ...
          entries: [...children].sort((a, b) => a.localeCompare(b))
            .map(name => ({
              name, path: target === '/' ? `/${name}` : `${target}/${name}`, isDirectory: true, hidden: name.startsWith('.'),
            })),
          truncated: false,
        })
      },
