...
describe('translate: defensive tool-call branches', () => {
  it('synthesizes a stable call id when deltas never carry id or name', async () => {
    const chunks = await collect(translate(feed(
      firstChunk,
    ...
    const end = chunks[2]
    expect(delta).toMatchObject({ type: 'tool-call-delta', index: 0, argumentsDelta: '{}' })
    expect(end).toMatchObject({
      type: 'block-end',
      index: 0,
      block: { type: 'tool-call', name: '', arguments: '{}' },
    })
    const endId = (end as { block: { id: string } }).block.id
    expect(endId).toMatch(/^call-.+/)
    // Delta and close carry the same synthesized id, so streamed and logged
    // views of the call agree.
    expect((delta as { id: string }).id).toBe(endId)
    expect(chunks[3]).toEqual({ type: 'finish', reason: { kind: 'tool-calls' } })
  })

  it('synthesizes distinct call ids for multiple id-less tool calls', async () => {
    const chunks = await collect(translate(feed(
      firstChunk,
    ...
    const ends = chunks.filter(chunk => chunk.type === 'block-end')
    const ids = ends.map(chunk => (chunk.block as { id: string }).id)
    expect(ids).toHaveLength(2)
    expect(ids[0]).not.toBe(ids[1])
    expect(ids[0]).toMatch(/^call-.+/)
  })

  it('treats an empty-string provider id as missing', async () => {
    const chunks = await collect(translate(feed(
      firstChunk,
    ...
    expect((end?.block as { id: string }).id).toMatch(/^call-.+/)
  })
