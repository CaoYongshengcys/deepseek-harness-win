 ...
 * while ensuring no chunk follows `finish`. A tool call whose id the provider omits or empties
 * receives a synthesized id at block close, so the assembled block always carries a non-empty
 * call id.
 *
...
/**
 * Return the block's call id, synthesizing a unique one the first time a
 * provider omits or empties it. Some OpenAI-compatible endpoints stream
 * tool-call deltas without any `id`; the harness requires a non-empty call id
 * to pair a tool call with its result, so the synthesized id stands in for the
 * provider's missing one and stays stable across the block's deltas and close.
 * @param block - an open tool-call block.
 * @returns the provider's id when present, otherwise the synthesized stand-in.
 */
function ensureCallId(block: OpenBlock): string {
  if (!block.callId) block.callId = `call-${crypto.randomUUID()}`
  return block.callId
}
