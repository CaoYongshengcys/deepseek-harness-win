...
import type {
  DirectoryListing, FileListing, IWorkspaces, SessionId, SnapshotStore, WorkspaceId, WorkspaceListState, WorkspaceView,
...
export class TestWorkspaces implements IWorkspaces {
  ...
  /**
   * File-tree listing (recorded). The default serves an empty level at the
   * host working directory; stub to shape a tree.
   * @param path - absolute directory to list; absent lists the host working directory.
   * @param signal - aborts a superseded scan (recorded and forwarded like the production face).
   * @returns the level's file and directory entries.
   */
  async listFiles(path?: string, signal?: AbortSignal): Promise<FileListing> {
    ...
    return { path: path ?? '/home/test', entries: [], truncated: false }
  }
